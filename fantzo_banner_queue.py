...
def _set_setting(key, value):
    ...
    with core.db() as conn:
        conn.execute(
            "INSERT INTO live_tv_banner_settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value",
            (key, str(value)),
        )
...
def add_banner(file_id, caption=""):
    ensure_tables()
    with core.db() as conn:
        conn.execute(
            "INSERT INTO live_tv_banners(file_id,caption,status,created_at) VALUES(?,?, 'queued', ?)",
            (file_id, caption or "", _now_iso()),
        )
...
def next_banner():
    ...
    with core.db() as conn:
        return conn.execute(
            "SELECT id,file_id,caption FROM live_tv_banners WHERE status='queued' ORDER BY id ASC LIMIT 1"
        ).fetchone()
...
def clear_queue():
    ensure_tables()
    with core.db() as conn:
        conn.execute("DELETE FROM live_tv_banners WHERE status='queued'")


def admin_keyboard():
    paused = _setting("paused", "0") == "1"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("▶ Resume" if paused else "⏸ Pause", callback_data="banner_toggle")],
...
async def _post_next(bot):
    ...
    if not row:
        return False
    caption = str(row["caption"] or "").strip() or DEFAULT_CAPTION
...
async def pause(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user and update.effective_user.id == core.ADMIN_USER_ID:
        _set_setting("paused", "1")
        await update.effective_message.reply_text("⏸ Live TV automatic banner posting paused.")


async def resume(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user and update.effective_user.id == core.ADMIN_USER_ID:
        _set_setting("paused", "0")
        await update.effective_message.reply_text("▶ Live TV automatic banner posting resumed.")


async def clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user and update.effective_user.id == core.ADMIN_USER_ID:
        clear_queue()
        await update.effective_message.reply_text("🗑 Unposted Live TV banner queue cleared.")
...
async def scheduler_loop(application):
    ...
    while True:
        try:
            ...
            logger.exception("Fantzo Live TV banner scheduler error")
