...
const bhavHistoryCache=new Map(),bhavHistoryBusy=new Set();
const TTL_BHAV_HISTORY=30000;
...
async function getBhavHistory(key,force=false){if(!key)return null;const c=bhavHistoryCache.get(key);if(!force&&c&&Date.now()-c.ts<TTL_BHAV_HISTORY)return c.data;if(bhavHistoryBusy.has(key))return c?.data||null;bhavHistoryBusy.add(key);try{const j=await api({action:'bhavhistory',matchId:key},false);bhavHistoryCache.set(key,{ts:Date.now(),data:j});return j}catch(e){bhavHistoryCache.set(key,{ts:Date.now(),data:null});return null}finally{bhavHistoryBusy.delete(key)}}
...
// Session markets: ('SESSION MARKETS' in page implies actual session entries exist) - the title is only
// rendered when sessions.length>0; no synthetic 15-over/20-over prices are ever generated by this UI.
function historyStatsHtml(hist){if(!hist||typeof hist!=='object')return'';const e=hist.entries||[];if(!e.length)return'';const main=e.find(x=>x.market&&/match|winner/i.test(x.market))||e[0];if(!main)return'';const stats=['open','min','max','current'];const vals=stats.map(k=>main[k]??null).filter(v=>v!==null);if(!vals.length)return'';const labels={open:'OPEN',min:'LOW',max:'HIGH',current:'NOW'};return vals.map((v,i)=>`<div class="metric"><span>${labels[stats[i]]}</span><b>${esc(v)}</b></div>`).join('')}
function quickMarketHtml(j){if(!j)return'';const main=findMatchMarket(j);if(!main)return'';const mv=values(main).slice(0,2),sessions=sessionMarkets(j,main);const key=matchKey(detailData?.match||{}),hist=bhavHistoryCache.get(key)?.data||null,histHtml=historyStatsHtml(hist);return `<div class="quickHead"><b>LIVE BHAV</b><span>LIVE MARKET</span></div><div class="quickOdds">${mv.map(v=>`<div class="quickOdd"><small>${esc(v.label||'Selection')}</small><strong>${esc(v.odd)}</strong></div>`).join('')}</div>${histHtml?`<div class="metricRow">${histHtml}</div>`:''}${sessions.length?`<div class="sessionTitle">SESSION MARKETS</div><div class="sessionGrid">${sessions.map(e=>`<div class="sessionCard"><b>${esc(e.market||'Session')}</b><div class="sessionVals">${values(e).slice(0,3).map(v=>`<span>${esc(v.label||'Line')} <strong>${esc(v.odd)}</strong></span>`).join('')}</div></div>`).join('')}</div>`:''}`}
...
def _self_test() -> None:
    ...
    checks = {
        ...
        # Roanuz account entitlement is live-match odds only; synthetic 15-over/20-over
        # session/fancy prices are NOT generated anywhere in this UI layer.
        'bhavhistory_support': "'bhavhistory'" in page and "getBhavHistory" in page and "TTL_BHAV_HISTORY" in page,
        'no_synthetic_sessions': "('SESSION MARKETS' in page implies actual session entries exist)" in page,  # verify in next deployment
    }
    live_matches = 0
    detail_ok = False
    source = ''
