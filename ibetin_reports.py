...
def _report_liveline():
    ...
    rows = []
    for uid, p in sorted(phones.items(), key=lambda item: str(item[1].get("verified_at") or ""), reverse=True):
        ...
        rows.append([
            ...
            p.get("verified_at") or "",
            users.get(uid, {}).get("last_seen") or business.get(uid, {}).get("last_seen") or "",
        ])
    return (
